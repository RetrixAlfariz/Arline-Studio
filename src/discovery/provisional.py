...
def _ensure_schema(service: DiscoveryService) -> None:
    marker = getattr(service, "_provisional_schema_ready", False)
    if marker:
        return
    backup_sqlite_before_migrations(
        service.store.path, {"discovery_provisional_meta": PROVISIONAL_SCHEMA_VERSION}
    )
...
def bind_workspace_edit_hooks(service: DiscoveryService) -> None:
    ...
    original_update_variant = workspace.update_variant
    original_update_family = workspace.update_entity_family

    def update_variant_with_discovery(variant_id: str, *, note: str = "updated", **changes: Any):
        before = workspace.get_variant(variant_id)
        result = original_update_variant(variant_id, note=note, **changes)
        if not str(note or "").startswith("discovery:"):
            try:
                record_user_variant_edit(service, before, result, changes)
            except Exception:
                pass
        return result

    def update_family_with_discovery(family_id: str, *, note: str = "updated", **changes: Any):
        before = workspace.get_entity_family(family_id)
        result = original_update_family(family_id, note=note, **changes)
        if not str(note or "").startswith("discovery:") and changes.get("name") and changes.get("name") != before.get("name"):
            links = _resource_subject_links(service, family_id)
            ...
            if keys:
                ...
                with service.store._lock, service.store.connection() as con:
                    con.execute(
                        f"UPDATE discovery_propositions SET subject_label=?,updated_at=? WHERE subject_key IN ({marks})",
                        [result["name"], utc_now(), *keys],
                    )
                    con.execute(
                        f"UPDATE discovery_propositions SET object_label=?,updated_at=? WHERE object_key IN ({marks})",
                        [result["name"], utc_now(), *keys],
                    )
    ...
    workspace.update_variant = update_variant_with_discovery
    workspace.update_entity_family = update_family_with_discovery
