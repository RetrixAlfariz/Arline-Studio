...
def attach_discovery(router, *, memory_service, foundation=None) -> DiscoveryService:
    """Attach Narrative Discovery to the existing Memory router/application instance."""
    ...
    if isinstance(existing, DiscoveryService):
        ...
        discovery = DiscoveryService(
            ...
            foundation=foundation or memory_service.foundation,
        )
    ...
    if not getattr(memory_service, "_v121_discovery_hooks_bound", False):
        history = memory_service.history
        foundation_store = foundation or memory_service.foundation
        ...
        original_trash = getattr(foundation_store, "trash", None)
        original_restore = getattr(foundation_store, "restore", None)
        ...
        def feedback_with_discovery(turn_id: str, **kwargs):
            ...
            try:
                ...
                if status == "accepted":
                    discovery.store.set_source_kind_active(turn_id, "user_edited_prose", active=False, reason="feedback_replaced")
                    discovery.capture_turn(turn_id, source_kind="accepted_generation")
                elif status == "edited_accept":
                    discovery.store.set_source_kind_active(turn_id, "accepted_generation", active=False, reason="feedback_replaced")
                    discovery.capture_turn(turn_id, source_kind="user_edited_prose")
                elif status == "rejected":
                    discovery.store.set_source_kind_active(turn_id, "accepted_generation", active=False, reason="feedback_rejected")
                    discovery.store.set_source_kind_active(turn_id, "user_edited_prose", active=False, reason="feedback_rejected")
            except Exception as exc:
        ...
        def delete_scope_with_discovery(*, project_id=None, world_id=None, branch_id=None):
            ...
            return original_delete_scope(project_id=project_id, world_id=world_id, branch_id=branch_id)
        ...
        if callable(original_restore):
            def restore_with_discovery(resource_type: str, resource_id: str, **kwargs):
                ...
                if resource_type == "session":
                    try:
                        # Restore only lifecycle-invalidated evidence; user-dismissed
                        # propositions and source-revision decisions remain separate.
    ...
    def dismiss_discovery(proposition_id: str, payload: DiscoveryDecisionPayload):
        try:
            return discovery.dismiss(
                proposition_id,
                _context(project_id=payload.project_id, world_id=payload.world_id,
                         branch_id=payload.branch_id, session_id=payload.session_id),
                note=payload.note,
            )
        except KeyError as exc:
